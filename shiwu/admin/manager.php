...
<form name="manager" id="manager" method="post" >
	<table id="cont" cellspacing="0" cellpadding="0">
		<tr>
			<td class="k"><input name="all"  type="checkbox"  value="全选"  onclick="selectAll(document.manager)"></td>
		...
		<?php
            ...
            foreach ($result as $rs) {
				...
				echo "</td>";
				echo "<td><a href='set.php?id=".$rs["p_name"]."&type=".$rs["p_type"]."'>编辑</a></td>";
				echo "</tr>";
            }
		?>

		<tr>
			<td colspan="6"><button type='submit' onclick="recive()">放入回收站</button><button type='submit' onclick="abso_delete()">彻底删除</button></td>
		</tr>
...
function recive()  
{  
document.manager.action="recive_pro_ok.php";    
}  
function abso_delete()  
{  
document.manager.action="delete.php";  
} 
 </script>
</body>
</html>