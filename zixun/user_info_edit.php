...
$sqlb="select * from user_info where user_id={$userid} ;";
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $sqla="update user_info set name='{$nnam}',sex='{$nsex}',birth='{$nbirth}',
address='{$nadd}',telcode='{$nint}' where user_id={$userid};";
    $resa=mysqli_query($dbc,$sqla);

    $sqlc="update user_card set name='{$nnam}' where user_id={$userid};";
    $resc=mysqli_query($dbc,$sqlc);
    if($resa==1&&$resc==1)
    {

        echo "<script>alert('修改成功！')</script>";
        echo "<script>window.location.href=uuser_info.phpcript>";
